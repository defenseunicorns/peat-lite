...
build-android-target:
	...
	cp android-ffi/target/$(TARGET)/release/libhive_lite_android.so android/src/main/jniLibs/$(ABI)/

# Build native libraries for all Android ABIs
build-android:
	@echo "Building hive-lite-android native libraries..."
	...
	@ls -la android/src/main/jniLibs/*/libhive_lite_android.so 2>/dev/null || echo "  (none found)"
...
publish-local: generate-bindings
	...
	@echo ""
	@echo "Published to ~/.m2/repository/com/revolveteam/hive-lite/"
	@ls -la ~/.m2/repository/com/revolveteam/hive-lite/ 2>/dev/null || echo "  (not found)"
...
# Pre-commit hook target (for Codex.md compliance)
...
clean:
	cargo clean
	rm -rf android/src/main/jniLibs/*/libhive_lite_android.so
	rm -rf android/build
