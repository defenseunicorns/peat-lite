...
use eche_lite::{Header, decode_header, encode_header, HEADER_SIZE, MAX_PAYLOAD_SIZE};
use heapless::Vec;

// Re-export shared types so existing `use super::message::*` keeps working.
pub use eche_lite::{CrdtType, MAX_PACKET_SIZE, MessageError, MessageType};
...
impl Message {
    ...
    pub fn data_with_ttl(
        ...
        let mut msg = Self::new(MessageType::Data, node_id, seq_num);
        msg.flags = eche_lite::FLAG_HAS_TTL;
        msg.payload.push(crdt_type).ok()?;
        msg.payload.extend_from_slice(crdt_data).ok()?;
        msg.payload
